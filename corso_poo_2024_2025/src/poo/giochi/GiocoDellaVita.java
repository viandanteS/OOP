...
public class GiocoDellaVita{
	...
	public GiocoDellaVita( int n, int m ){
		if( n<=0 || m<=0 ) throw new IllegalArgumentException(); //segnala eccezione se n ed m sono invalidi
		this.n=n; this.m=m;
		mappa=new char[n][m];
		nuovaMappa=new char[n][m];
		for(int i=0; i<n; i++)	for(int j=0; j<m; j++) mappa[i][j]='.'; //territorio vuoto
	}
	...
	public void prossimaGenerazione(){
		for( int i=0; i<n; i++ )
			for( int j=0; j<m; j++ ){
				int v=vicini( i, j );
				if( mappa[i][j]=='*' ) nuovaMappa[i][j]=( v==2 || v==3 ) ? '*' : '.';
	...
	public static void main( String[] args ){//test
		...
		System.out.println(gol);gol.aggiungiOrganismo(0,2); gol.aggiungiOrganismo(0,5);
		gol.aggiungiOrganismo(0,6); gol.aggiungiOrganismo(1,0); gol.aggiungiOrganismo(2,3);
		gol.aggiungiOrganismo(2,5);gol.aggiungiOrganismo(3,0); gol.aggiungiOrganismo(3,4);
		System.out.println(gol);
		gol.prossimaGenerazione();
		System.out.println(gol);
		gol.prossimaGenerazione();
		System.out.println(gol);
		System.out.println("Ora con configurazione random");
		gol.configuraRandom();
		System.out.println(gol);
		for( int i=0; i<5; ++i ){
			gol.prossimaGenerazione();
